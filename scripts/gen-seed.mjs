// Generates migrations/seed.sql from data/products-master.json + assets/products.js + assets/i18n.js
...
function loadFrontendData() {
  ...
  return {
    ...
    MASTER: sandbox.window.BTT_PRODUCT_MASTER || [],
  };
}
...
function main() {
  const { I18N, MASTER } = loadFrontendData();
  ...
  lines.push("");
  lines.push("-- Clean old products & i18n");
  ...
  MASTER.forEach((p, i) => {
    // We insert primary key as slug and also legacyId for compatibility
    const idsToSeed = [p.slug, p.legacyId];

    idsToSeed.forEach((id) => {
      lines.push(
        `INSERT OR REPLACE INTO products (id, category, look, price_now, price_old, default_size, active, sort) VALUES ` +
          `(${q(id)}, ${q(p.category)}, ${q(p.slug)}, ${n(p.price)}, 0, 0, 1, ${i});`,
      );

      for (const lang of LANGS) {
        const name = (p.name && (p.name[lang] || p.name.ru)) || p.model;
        const catLabel = (p.categoryName && (p.categoryName[lang] || p.categoryName.ru)) || p.category;
        const desc = (p.description && (p.description[lang] || p.description.ru)) || "";
        const sizes = [p.dimensions];
        const specs = {
          mat: p.materials.join(", "),
          dim: p.dimensions,
          maxLoad: p.maxLoad || "",
          colors: (p.confirmedColors || []).map((c) => c.name[lang] || c.name.ru).join(", "),
        };
      ...
      if (Array.isArray(p.images)) {
        p.images.forEach((imgKey, imgIdx) => {
          lines.push(
            `INSERT INTO media (key, product_id, alt, sort) VALUES (${q(imgKey)}, ${q(id)}, ${q(p.model)}, ${imgIdx});`,
          );
        });
      }
      lines.push("");
  ...
  console.log(`Wrote ${out} (${MASTER.length} SKUs × 2 aliases × ${LANGS.length} langs).`);
}
